...
class MockUseCaseProvider(Provider):
    ...
    def get_use_case(self) -> DeleteAnswerUseCase:
        mock_use_case = Mock()
...
async def use_case(container) -> Mock:
    return await container.get(DeleteAnswerUseCase)
...
@pytest.mark.asyncio
async def test_delete_answer(client: TestClient, use_case: Mock):
    question_id = str(uuid4())
    response = client.delete(f"/answers/{question_id}")
